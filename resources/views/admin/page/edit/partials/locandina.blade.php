        ...
        <div class="uk-accordion-content">
                            ...
                            <div class="uk-form-row" data-uk-grid-margin>
                                <div class="uk-grid">
                                    <div class="uk-width-medium-1-1 uk-row-first uk-margin-bottom">
                                        ...
                                        <div class="md-input-wrapper">
                                            <select id="tipologia_blocco" class="md-input tipologia_blocco"
                                                name="tipologia_blocco" onchange="mostraBlocco(this)">
                    ...
                    @foreach (json_decode($result->locandina) as $key => $value)
                        @php
                            if (isset($value->ordine) && !empty($value->ordine)) {
                                $ordine = $value->ordine;
                                unset($valu->ordine);
                            } elseif (!isset($value->ordine) || empty($value->ordine)) {
                                $ordine = '';
                                unset($value->ordine);
                            }
                        ...
                        <div data-repeater-item>
                            <div class="md-input-wrapper">
                                <div class="md-card-content contenitore_repetaer">
                                    <div class="uk-form-row" data-uk-grid-margin>
                                        <div class="uk-grid">
                                            <div class="uk-width-medium-1-1 uk-row-first uk-margin-bottom">
                                                ...
                                                <div class="md-input-wrapper">
                                                    <select id="tipologia_blocco" class="md-input tipologia_blocco"
                                                        name="tipologia_blocco" onchange="mostraBlocco(this)" disabled>
                                                        <option value="" required>Seleziona ...</option>
                                                        <option value="personaggio-interprete"
                                                            {{ $template == 'personaggio-interprete' ? 'selected' : '' }}>
                                                            Personaggio - Interprete</option>
                                                        <option value="etichetta-personaggi"
                                                            {{ $template == 'etichetta-personaggi' ? 'selected' : '' }}>
                                                            Etichetta - Persone</option>
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    @include(
                                        'admin.page.edit.partials.' . $template,
                                        compact(reset($valori), 'key', 'ordine'))
                                    @unset($ordine_apparizione)
                                </div>
                            </div>
                            <div
                                class="uk-width-medium-1-1 uk-row-first uk-container uk-height-1-1  uk-container-center">
                                <input data-repeater-delete
                                    class="uk-button uk-width-1-1 uk-button uk-button-danger uk-vertical-align-middle"
                                    type="button" value="Elimina Blocco Locandina" />
                            </div>
                        </div> --}}
                    @endforeach
                @endif
            ...
            <div class="md-input-wrapper">
                <div class="md-card-content">
                    <input data-repeater-create class="uk-button uk-width-1-1 uk-button uk-button-success uk-margin-top"
                        type="button" value="Aggiungi blocco" />
                </div>
            </div>
        </div>
    </div>
                ...
                $(".suggest_termini")
                    .autocomplete({
                        ...
                        select: function(event, ui) {
                            ...
                            var numero = $(divcontiner).find('#termini_selezionati').children('div')
                                .length + 1;
                            var divToAdd = '<div id="' + ui.item.id +
                                '" class="uk-alert uk-alert-info nome_persona"><input type="hidden" name="' +
                                nomeCampo + '" value="' + ui.item.id + '">' + ui.item.value + '</div>';
